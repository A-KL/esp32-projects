...
const int screen_width = TFT_WIDTH; // px, width of screen
const int screen_height = TFT_HEIGHT; // px, height of screen
...
const int col_width = 4; // px, width of each column
...
void screen_Task_routine(void *pvParameters){
    ...
    while(true)
    {
        ...
        tft.drawFastHLine(0, screen_height-1, screen_width, TFT_WHITE);

        const int col_px = screen_width/N_columns;

        for(int i = 0; i < N_columns; i++){
            int length = y[i] * 2 *((float)screen_height/(dB_max-dB_min));
            tft.fillRect(i*col_px-col_width, screen_height-length, col_width, length, TFT_WHITE);
        }
...
void comp_Task_routine(void *pvParameters)
{
   ...
   // i2s_adc_enable(I2S_NUM_0);
    ...
    while(true){
        ...
        if(!timestamps[0]) 
            timestamps[0] = micros(); // run-once timestamping
