...
static esp_err_t i2c_trans(i2c_port_t i2c_num, uint8_t addr, uint8_t *txdata, uint8_t txlen)
{
    esp_err_t rc;
...
static esp_err_t i2c_recv(i2c_port_t i2c_num, uint8_t addr, uint8_t *rxdata, uint8_t rxlen)
{
    esp_err_t rc;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
...
esp_err_t wm8960_init()
{
  ...
  write_register_i2c(CODEC_ADDR,0x28, 0x0074 | 0x0100); //Left Speaker Volume
  write_register_i2c(CODEC_ADDR,0x29, 0x0074 | 0x0100); //Right Speaker Volume
...
esp_err_t wm8960_set_vol(uint8_t vol)
{
    esp_err_t ret = 0;
    ...
    ret |= write_register_i2c(CODEC_ADDR, 0x0a, 0x100 | vol_to_set);
    ret |= write_register_i2c(CODEC_ADDR, 0x0b, 0x100 | vol_to_set);

    return ret;
}

esp_err_t wm8960_set_hp_vol(uint8_t vol)
{
    esp_err_t ret = 0;
    // int vol_to_set = 0;
    // if (vol == 0) {
    ...
    //     vol_to_set = (vol / 10) * 5 + 200;
    // }
    // ret |= write_register_i2c(CODEC_ADDR, 0x02, 0x100 | vol_to_set);
    // ret |= write_register_i2c(CODEC_ADDR, 0x03, 0x100 | vol_to_set);
